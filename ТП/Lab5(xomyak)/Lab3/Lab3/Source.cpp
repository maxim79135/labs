...
#pragma comment(lib, "Neyavn.lib")
extern "C" __declspec(dllimport) char minDistance(char* Buffer, int count, int verA, int verB);
	...
	case WM_COMMAND:
		...
		case ButtonFirst:
			for (int i = 0; i < count * count; i++)
				SendMessage(hEdit[i], WM_ACTIVATE, 0, 0);
			SendMessage(hwndComboBox, WM_SHOWWINDOW, 0, 0);
			SendMessage(hwndButton3, WM_SHOWWINDOW, 0, 0);
			SendMessage(hwndButton4, WM_SHOWWINDOW, 0, 0);
		...
		case ButtonTo:
			...
			MYPROC ProcAdd;
			hinstLib_1 = LoadLibrary(L"Yavn.dll");
			if (hinstLib_1 != NULL)
				ProcAdd = (MYPROC)GetProcAddress(hinstLib_1, "minDistance");
			else {
				MessageBox(hwnd, L"Не найдена DLL", L"Ошибка", MB_OK);
				break;
			}
			if (ProcAdd != NULL) {
